  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap", alignItems:"center" }}>
        <span style={{color:"rgba(255,255,255,0.45)",fontSize:12,letterSpacing:.5}}>
          {t.roomShapeTitle /* ex: "FORME DE LA SALLE" */}
        </span>
        <div style={{flex:1}}/>
        <Btn
          small
          variant={mode==="draw"?"primary":"ghost"}
          onClick={() => { setMode(mode==="draw"?"view":"draw"); setDrawing([]); }}
        >
...
function FloorPlan({ ev, onUpdateTables, onSelectTable, selectedTable, highlightAvailable, onDropGuestToTable }) {
  ...
  const handleMouseMove = useCallback((e) => {
    if (!dragging) return;
    const pt = getSVGPoint(e);
    onUpdateTables(ev.tables.map(tbl => {
      if (tbl.id === dragging.tableId) {
        return {
          ...tbl,
          x: Math.max(50, Math.min(CANVAS_W-50, pt.x - dragging.offsetX)),
          y: Math.max(50, Math.min(CANVAS_H-50, pt.y - dragging.offsetY))
        };
      }
      return tbl;
    }));
      ...
      {ev.tables.map(tbl => {
        ...
        return (
          <g
            ...
            onClick={() => onSelectTable(tbl.id === selectedTable ? null : tbl.id)}
            onDragOver={e => { e.preventDefault(); e.currentTarget.style.filter="drop-shadow(0 0 12px #C9973A88)"; }}
          ...
          >
            <title>
              {t.floorPlanTableTitle(tbl, seated)}
            </title>
            ...
            {(() => {
              ...
              return tbl.shape !== "rect" && pct > 0 ? (
                <circle
                  cx={tbl.x} cy={tbl.y} r={r}
                  fill="none" stroke={fillCol} strokeWidth="3"
            ...
            <text x={tbl.x} y={tbl.y-4} textAnchor="middle" fill={col} fontSize="15" fontWeight="700" fontFamily="Georgia,serif" style={{pointerEvents:"none"}}>
              {tbl.number}
            </text>
            <text x={tbl.x} y={tbl.y+13} textAnchor="middle" fill={col} fontSize="10" fontFamily="Georgia,serif" opacity=".8" style={{pointerEvents:"none"}}>
              {seated.length}/{tbl.capacity}
            </text>
            {tbl.label && (
              <text x={tbl.x} y={tbl.y+27} textAnchor="middle" fill={col} fontSize="9" fontFamily="Georgia,serif" opacity=".6" style={{pointerEvents:"none"}}>
                {tbl.label}
              </text>
            )}
