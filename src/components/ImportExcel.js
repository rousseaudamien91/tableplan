...
function ImportExcel({ open, onClose, onImport, existingGuests }) {
  const { t } = useI18n();
      ...
      {step === "upload" && (
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <div
            ...
            style={{
              border:`2px dashed ${theme.primary}33`,
              borderRadius:14,
              padding:"40px 20px",
              textAlign:"center",
              cursor:"pointer",
          ...
          >
            <div style={{ color:theme.primary, fontWeight:700, marginBottom:8 }}>
              📋 {t.importExpectedFormat}
            </div>
            <code style={{ color:theme.text, display:"block", marginBottom:4 }}>
              {t.importExampleHeader}
            </code>
            <code style={{ color:theme.textMuted, display:"block" }}>
              {t.importExampleRow}
            </code>
            <div style={{ marginTop:8 }}>{t.importAutoDetect}</div>
          </div>

          {/* CSV MODEL */}
          <a
            href={
              "data:text/csv;charset=utf-8,Nom,Email,R%C3%A9gime,Table,Notes%0ADupont%20Marie,marie@ex.fr,standard,1,VIP%0AMartin%20Paul,paul@ex.fr,v%C3%A9g%C3%A9tarien,2,"
            }
      ...
      {step === "preview" && (
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            ...
            >
              ✅ <strong style={{ color:"#27AE60" }}>{guests.length}</strong>{" "}
              {t.importDetected}
            </div>
            ...
            {duplicates.length > 0 && (
              ...
              >
                ⚠️ <strong style={{ color:"#F0C97A" }}>{duplicates.length}</strong>{" "}
                {t.importDuplicates}
              </div>
            )}
          </div>
          ...
          >
            ...
            >
              <thead>
                <tr style={{ background:theme.card, position:"sticky", top:0 }}>
                  ...
                  return (
                    ...
                    >
                      <td
                        style={{
                          padding:"7px 12px",
                          color: isDup ? theme.warning : theme.text,
                        }}
                      >
                        {isDup ? "⚠️ " : ""}
                        {g.name}
